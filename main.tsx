...
import Ops from './components/pages/Ops';
...
type QuickCreateType = 'customer' | 'task' | 'service';
type DrilldownType = 'customer' | 'task' | 'service';
type DrilldownFocus = 'overview' | 'tasks' | 'notes' | 'timeline' | 'details' | 'context';
type DrilldownTarget = { page: Page; entityType: DrilldownType; entityId: string; customerId?: string; focusSection?: DrilldownFocus };
...
export default function App() {
  ...
  const [quickCreate, setQuickCreate] = useState<null | QuickCreateType>(null);
  const [pendingDrilldown, setPendingDrilldown] = useState<DrilldownTarget | null>(null);
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<{ type?: QuickCreateType; source?: string }>).detail;
  ...
  const openDrilldown = useCallback((target: DrilldownTarget) => {
    setMobileMenuOpen(false);
    setPage(target.page);
    setPendingDrilldown(target);
  }, []);

  useEffect(() => {
    if (!pendingDrilldown || page !== pendingDrilldown.page) return;
    const detail = pendingDrilldown;
    const timer = window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent('crm:focus', { detail }));
      setPendingDrilldown(null);
    }, 40);
    return () => window.clearTimeout(timer);
  }, [page, pendingDrilldown]);
  ...
  const pageNode = useMemo(() => {
    ...
    switch (page) {
      ...
      case 'ops':
        return <Ops onDrilldown={openDrilldown} />;
      case 'einstellungen':
        return <Einstellungen />;
      default:
        return <Dashboard onNav={nav} search={search} />;
    }
  }, [page, nav, search, openDrilldown]);
